...
import com.zap.Base.DriverFactory;
import com.zap.Page.MyAppNavigation;
...
public class ZapSchedulingScanTest {
    ...
    private final static String CHROME_DRIVER_PATH = "D:/selenium jar files/chromedriver_win32/chromedriver.exe";
    ...
    public void testSecurityVulnerabilitiesBeforeLogin() {
        ...
        System.out.println("No of vulnerabilties when user is not logged in "+alerts.size());
    }
    ...
    public void testSecurityVulnerabilitiesAfterLogin() {
        ...
        List<Alert> alerts = filterAlerts(zapScanner.getAlerts());
        logAlerts(alerts);
        sa.assertEquals(alerts.size(), equalTo(0));
        System.out.println("No of vulnerabilties when user is  logged in and naviagted to the site "+alerts.size());
